...
{{/* Function */}}
{{if gt $pa 0}}
	{{if not ($arme.Get "pistol2")}}
		{{$arme.Set "pistol2" 1}}
		{{ $desc = (joinStr "" "Il reste 8/8 balles dans le pistolet secondaire.")}}
	{{else}}
		{{$arme.Set "pistol2" (add ($arme.Get "pistol2") 1)}}
		{{$y := $arme.Get "pistol2"}}
		{{$x := sub 8 $y}}
	  	{{if lt (toFloat $y) (toFloat 8)}}
				{{ $desc = (joinStr "" "Il reste " (toString (toInt $x)) "/8 balles dans le pistolet secondaire.")}}
